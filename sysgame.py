...
#初始化
q=queue.Queue()
startmenulist=['新游戏','加载游戏','退出','关于我们']
startmenutitle='开始游戏'
...
#类定义
class menu:
    def init(self):#在每次判断键盘进入成立条件后调用
        global loop
        loop=True
...
while True:
    ...
    if loop==False:
            ...
            if openfile==1:
                ...
                while True:
                    ...
                    if not q.empty():
                        ...
                        keyboards=q.get()
                        if keyboards=='space' :
                            if ( a[games['PlayerX']+1][games['PlayerY']]==2 or (games['PlayerX']+1==games['ShotX'] and games['PlayerY']==games['ShotY'])):
                                for i in range(3):
                                    gameplayer.go(-1,0)
                                prints=1
                        if keyboards=='s':
        ...
        if startmenu.mode==3:
            startmenu.init()
            webbrowser.open('https://github.com/SueYanr/sysgames/blob/main/sysgames.py')
